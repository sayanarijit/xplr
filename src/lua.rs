...
mod tests {
    ...
    fn test_compatibility() {
        ...
        assert!(check_version("0.17.6", "foo path").is_ok());

        // Prev major release is ERR
        // - Not yet

        // Prev minor release is ERR (Change when we get to v1)
        assert!(check_version("0.16.6", "foo path").is_err());

        // Prev bugfix release is OK
        assert!(check_version("0.17.5", "foo path").is_ok());

        // Next major release is ERR
        assert!(check_version("1.17.6", "foo path").is_err());

        // Next minor release is ERR
        assert!(check_version("0.18.6", "foo path").is_err());

        // Next bugfix release is ERR (Change when we get to v1)
        assert!(check_version("0.17.7", "foo path").is_err());
    }
}
